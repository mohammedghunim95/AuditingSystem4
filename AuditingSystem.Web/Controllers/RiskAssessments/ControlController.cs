...
namespace AuditingSystem.Web.Controllers.RiskAssessments
{
    public class ControlController : Controller
    {
        ...
        public async Task<IActionResult> Add(int? riskIdentificationId)
        {
            ...
            ViewBag.ControlEffectivenessId = new SelectList(controlEffectivenesss.Select(r => new { Id = r.Id, Name = $"{r.Name} - {r.Rate}" }), "Id", "Name");
        ...
        public async Task<IActionResult> Edit(int id, int? riskIdentificationId)
        {
            if (riskIdentificationId.HasValue)
            {
               ...
               null);
                    ...
                    ViewBag.ControlEffectivenessId = new SelectList(controlEffectivenesss.Select(r => new { Id = r.Id, Name = $"{r.Name} - {r.Rate}" }), "Id", "Name");
                    ViewBag.RiskIdentificationId = new SelectList(riskIdentifications, "Id", "Name", controls.RiskIdentificationId);
            ...
            ViewBag.ControlEffectivenessId = new SelectList(controlEffectiveness.Select(r => new { Id = r.Id, Name = $"{r.Name} - {r.Rate}" }), "Id", "Name", control.ControlEffectivenessId);
